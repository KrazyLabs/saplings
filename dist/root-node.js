...
let RootNode = class RootNode {
  ...
  traverse(evaluation, wme) {
    if (!evaluation) {
      throw new Error('Missing required parameter evaluation for call to RootNode.traverse');
    }
  ...
  getChildByLabel(label) {
    ...
    this.children.forEach(child => {
      if (!foundChild) {
        foundChild = child.label === label ? child : child.getChildByLabel(label);
      }
    });
    return foundChild;
  }
